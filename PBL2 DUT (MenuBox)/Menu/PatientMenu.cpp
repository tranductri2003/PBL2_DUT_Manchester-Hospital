...
void PatientMenu::viewMedicalHistory(Patient &patient)
{
    ...
    LinkedList<string> appointments = getAllAppointmentOfID(patient.getID());
    if (appointments.empty() == true)
    {
        printError("You have no appointments!");
        cout << endl;
        system("pause");
        PatientMenu::menu(patient);
    }
